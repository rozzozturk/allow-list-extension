...
chrome.action.onClicked.addListener(async (tab) => {
  ...
  try {
    ...
    if (!validHosts.includes(url.hostname)) {
      console.log("[Keepnet] Not on valid host, opening welcome screen instead of auto-redirect...")
      
      const welcomeUrl = chrome.runtime.getURL('welcome.html')
      
      try {
        const existingTabs = await chrome.tabs.query({ url: welcomeUrl })
        
        if (existingTabs && existingTabs.length > 0) {
          // Var olan welcome tab'ini öne getir
          await chrome.tabs.update(existingTabs[0].id, { active: true })
          await chrome.windows.update(existingTabs[0].windowId, { focused: true })
        } else {
          // Yeni welcome tab'i aç
          await chrome.tabs.create({ url: welcomeUrl })
        }
      } catch (welcomeError) {
        console.error("[Keepnet] Failed to open welcome screen:", welcomeError)
      }
      
      // Otomatik başlatma yok; kullanıcı welcome ekranından bilinçli başlatacak
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  switch (request.action) {
    ...
    case 'startAssistantFromWelcome':
      (async () => {
        try {
          console.log("[Keepnet] Start requested from welcome screen")
          
          // Kullanıcı onayı sonrası aktif et
          globalState.isActive = true
          
          const targetUrl = 'https://security.microsoft.com/homepage'
          const createdTab = await chrome.tabs.create({ url: targetUrl })
          
          sendResponse({ ok: true, tabId: createdTab.id, url: targetUrl })
        } catch (error) {
          console.error("[Keepnet] Failed to start from welcome screen:", error)
